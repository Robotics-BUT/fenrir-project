...
The firmware is available here ![Firmware](https://github.com/Robotics-BUT/fenrir-project/blob/main/software/arduino_nano/main/main.ino).

This Arduino Nano program works as a motor controller that talks with a main computer (for example a Raspberry Pi) using I²C communication. The Nano is an I²C slave – it waits for messages from the master and answers when needed. When the master sends data, the Nano updates target speeds or control settings. When the master asks for data, the Nano sends back measurements like encoder counts or wheel speeds. This communication happens using interrupt callbacks, so it does not stop the main program loop. In this way, the Nano can control motors and communicate at the same time.
