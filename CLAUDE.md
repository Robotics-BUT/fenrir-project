...
> **Parking LIFTED 2026-05-27 — Merge 4 landed.** Appendix C Merge 4
> (`modernization/phase-3 → main`) was merged and pushed on 2026-05-27, so
> `main` now carries the Phase 1 `rgb_leds_handler` fix (T1.9) + the Phase 3
> prep CLAUDE.md. The `modernization/phase-4` simulator branch is the only
> one still open here; its Merge 6 PR is ready (see
> `bpc-prp-devel/MODERNIZATION_ROADMAP.md` Appendix C).

- **`main`** is the pushed baseline; carries Merges 1–4 (T1.9 + Phase 3 prep).
- **`modernization/phase-1`** and **`modernization/phase-3`** — merged into `main` via Merge 4 (2026-05-27); slated for deletion in the post-merge cleanup.
- **`modernization/phase-4`** (branched 2026-05-20 off phase-3) carries the **Phase 4 Gazebo simulator** — the `simulation/` package (`fenrir_sim`), `docker/sim/Dockerfile`, and the worlds. This is the active branch. Line, corridor and maze worlds all run end-to-end against the `/bpc_prp_robot/*` contract, and **all seven bridges are done (T4.3 complete, 2026-05-28)**: motor, lidar, line_sensor, encoder, ultrasound, button, rgb_leds. Student-facing how-to docs live in `BPC-PRP:modernization/phase-4` (`src/5_simulation/`).
...
| `fenrir_sim/*_bridge.py` | Seven bridge nodes that translate Gazebo topics ↔ the `/bpc_prp_robot/*` contract (T4.3 complete): `motor_bridge`, `lidar_bridge`, `line_sensor_bridge`, `encoder_bridge`, `ultrasound_bridge`, `rgb_leds_bridge` (all spawned by the launch files) and `button_bridge` (interactive keyboard reader — run via `ros2 run`, not from launch). `current_probes` is the one remaining unmodelled contract topic |
